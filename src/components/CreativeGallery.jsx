            ...
            <div className="creative-viewer-inner">
                <header className="creative-viewer-header">
                    <div><p className="label">{labels[item.type]} / {subtitle || 'Creative archive'}</p>
                        <h2 id="creative-viewer-title">{title}</h2></div>
            ...
            <div className="creative-grid">
                {visible.map((item, index) => {
                    ...
                    return <button key={item.slug} className={`creative-tile creative-tile--${item.type}`}
                        onClick={event => { opener.current = event.currentTarget; setActive(item); }} aria-label={`Open ${item.title}`}>
                        ...
                        <span className="creative-tile-title"><strong><Title title={title} /></strong><small>{subtitle || (item.type === 'audio' ? 'A sound study' : 'Creative study')}</small></span>
